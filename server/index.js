...
app.get('/products/:product_id/styles', async (req, res) => {
    try {
        const product = await pool.query(`SELECT DISTINCT ON (product_id) product_id,(SELECT json_agg(styles) 
        ...
        FROM skus WHERE style_id=6) as skus FROM styles WHERE product_id=($1))as styles) as results FROM styles WHERE product_id=($1)`,
       [req.params.product_id])
        res.json(...product.rows)
    } catch (err) {
       console.error(err.message)
