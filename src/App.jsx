...
function App() {
  ...
  //   }
  //   axios.get(`http://localhost:5000/products/${currentItemId}`).then((res)=>{
  //     item = res.data;
  //   }).then(()=>{
  //     axios.get(`http://localhost:5000/products/${currentItemId}/styles`).then((res)=>{
  //       item.style = res.data.results;
  //     })
  //   }).then(()=>{
  //     axios.get(`http://localhost:5000/products/${currentItemId}/related`).then((res)=>{
  //       item.related = res.data;
  ...
  useEffect(()=> {
    const fetchData = async () => {
      try {
        //api call
        const response = await axios.get(`http://localhost:5000/products/${currentItemId}`);
    ...
    const fetchItem = async () => {
      try {
        if (currentItemId) {
          // api call
          const response = await axios.get(`http://localhost:5000/products/${currentItemId}`);
          const getStyle = await axios.get(`http://localhost:5000/products/${currentItemId}/styles`);
          const getRelated = await axios.get(`http://localhost:5000/products/${currentItemId}/related`);
